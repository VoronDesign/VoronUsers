...
|  | [Bigtreetech Octopus Pro Color PIN Diagram](./GadgetAngel/Bigtreetech_Octopus_Pro_Color_PIN_Diagram) | A PDF file (and a JPG file) that has been colorized from the original PIN diagram so that the user can easily identify which endstops belong to which motors. Also, this PDF file labels all connectors instead of using "J" names. URL links have also been embedded in the PDF file so that the user can find additional information.  | :x: | :heavy_check_mark: | :heavy_check_mark: | :x: | :heavy_check_mark: |
